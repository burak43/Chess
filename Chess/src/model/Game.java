		...
		for( int i = 0; i < 8; i++)
		{
			for(int j = 0; j < 8; j++)
			{
				...
				if (gameManager.board.getSquareAtLoc(i, column).getPlacedPiece() != null) {
					...
					// if you want, you can also get the piece colour with the following
					// but it returns a Color type so need to convert to string
					//result[count][3] = gameManager.board.getSquareAtLoc(i, column).getPlacedPiece().getPieceColor();
					count++;
